...
def run(existingweights=False, modelfile='', existingdataset=False, datasetfile='', epochs=500, save=False, add3d=False):
    if existingweights:
        ...
        train.train(dataset, epochs, save=save, add3d=add3d)
...
if __name__ == '__main__':
    ...
    if args.weights:
        ...
        if args.dataset:
            run(existingdataset=True, datasetfile=args.dataset, epochs=args.epochs[0], save=args.timelapse, add3d=args.plot3d)
        else:
            run(epochs=args.epochs[0], save=args.timelapse, add3d=args.plot3d)
