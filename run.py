...
def run(existingweights=False, modelfile='', existingdataset=False, datasetfile='', save=False, add3d=False):
    if existingweights:
        model = train.create_mlp(2)
        model.load_weights(modelfile[0])
        ...
        train.train(dataset, save=save, add3d=add3d)
...
if __name__ == '__main__':
    ...
    if args.weights:
        run(True, args.weights, save=args.timelapse,
            add3d=args.plot3d)  # TODO add ability to use axes from existing dataset
    else:
        if args.dataset:
            run(existingdataset=True, datasetfile=args.dataset, save=args.timelapse, add3d=args.plot3d)
        else:
